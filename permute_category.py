...
def sort_input_arr_by_color(elem_arr):
...
def construct_arr_info(input_arr):
    final_result = []
    sorted_color_lists = sort_input_arr_by_color(input_arr)
    print ' sorted color list is : '
    print sorted_color_lists
    pre_cate = None
    pre_num = 0
    for sort_color_arr in sorted_color_lists:
        category_in_color_arrs = get_category_list(sort_color_arr)
        arranged_result, pre_cate, pre_num = permute_in_category_list(category_in_color_arrs, 8, pre_cate, pre_num)
        final_result.extend(arranged_result)
    return final_result


def permute_in_category_list(category_lists, threshold, pre_cate=None, pre_num=0):
    if len(category_lists) < 2:
        arranged_result = category_lists[0]
        now_cate = category_lists[0][0][1]
        now_num = len(arranged_result)
    ...
    while not_completed:
        ...
        for i in range(1, len(key_list)):
            ...
            if is_need_sep:
                ...
                current_need_step = len(category_lists[current_index]) - offset_arr[current_index]
...
def preprocess_arrange(category_lists, cate_index_map, key_list, threshold, pre_cate=None, pre_num=0):
    ...
    key_list[1][1] -= 1
    return cate_index_map, sorted(key_list, key=get_rate), arranged_result
...
def get_permute_key_split(key_list):
    ...
    max_average = int(math.ceil(float(key_list[0][1]) / (sum_value + 1)))
    return max_average
...
if __name__ == '__main__':
    ...
    print "input list is : "
    print input_data
    result = construct_arr_info(input_data)
    print " output list is : "
    print result
    print " input length is {0}".format(len(input_data))
    print " output length is {0} ".format(len(result))

    # for finding bad case
